...
- **UI & Component Constraints**:
  ...
  - **Zero Menu Obscuration & Z-Index Layering (Popup Menus Must Never Be Covered)**: Any popup menu, dropdown, listbox, combobox menu, or popover across the application (such as `AttachMenu`, `ModelSelect`, `CustomSelect`, `SlashMenu`, `SkillMenu`, `QuickModelPicker`, or context picker menus) MUST NEVER be obscured or covered by other UI elements, floating overlay contributions (e.g. `FloatingOverlayHost`, `TodoProgressBar` at `z-50`), toolbars, or sidebars. Popup menus must either be portaled via `createPortal(..., document.body)` with top-tier stacking elevation (`fixed z-[70]`), or their host containers must dynamically elevate to `z-[60]` / `z-[70]` while open, ensuring all interactive popup options remain fully visible and clickable without obstruction.
  - **Slash Commands i18n & Localization Support**: All slash commands in the prompt composer (such as `/compact`, `/model`, and any future built-in or plugin-contributed slash commands) MUST support internationalization (i18n). In non-English locales (e.g. Simplified Chinese `zh-CN`), slash command display names and descriptions in the slash suggestion menu must be localized (e.g. `/压缩` for compaction, `/模型` for model list). Input filtering and command execution MUST seamlessly accept both localized command tokens (e.g. `/压缩`, `/模型`) and their canonical English names / cross-language aliases (e.g. `/compact`, `/model`), preserving full backward compatibility and ensuring users can execute commands in either language.
