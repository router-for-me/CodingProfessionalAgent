 ...
 * Checks `.cpa/environments/environment.toml` first, then falls back to `.codex/environments/environment.toml`
 * and root `environment.toml`.
 */
export async function readProjectEnvironment(
    ...
    const resolvedBridge: ReadFileBridge | undefined =
        bridge || getHostBridge()
    if (!resolvedBridge) return null

    // 1. Try dedicated high-level RPC if available
 ...
 * Writes to `.cpa/environments/environment.toml` or existing target.
 */
export async function saveProjectEnvironment(
    ...
    const resolvedBridge: WriteFileBridge | undefined =
        bridge || getHostBridge()
